...
本项目展示了如何使用 **Jenkins + GitHub + Terraform** 在 **阿里云** 上实现端到端的自动化运维。从创建 ACK 集群，到部署监控系统（Prometheus + Grafana），再到自动构建和部署 Spring Boot 应用，并实现应用指标的可观测性。所有配置均通过代码管理，符合 GitOps 和 IaC 最佳实践。

### 🎯 核心目标
- 自动化创建 ACK 集群（包括 VPC、VSwitch）
